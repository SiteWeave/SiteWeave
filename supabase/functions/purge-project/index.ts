...
type StorageGroup = { bucket: string; paths: string[] }

function addStorageValue(
  buckets: Map<string, Set<string>>,
  value: unknown,
  fallbackBucket = 'message_files',
) {
  if (!value) return
...
async function collectProjectStoragePaths(
  ...
  const { data: photos, error: photosErr } = taskIds.length
    ? await supabaseAdmin
      .from('task_photos')
  ...
  for (const photo of photos || []) {
    ...
    addStorageValue(buckets, photo.storage_path, bucket)
    addStorageValue(buckets, photo.thumbnail_path, bucket)
  }

  const { data: issues, error: issuesErr } = await supabaseAdmin
    .from('project_issues')
    .select('id, before_photo_path, after_photo_path')
    .in('project_id', projectIds)
  throwOnError(issuesErr, 'select project issue photos')

  for (const issue of issues || []) {
    addStorageValue(buckets, issue.before_photo_path)
    addStorageValue(buckets, issue.after_photo_path)
  }

  const issueIds = (issues || []).map((issue) => issue.id)
  if (issueIds.length) {
    const { data: issueFiles, error: issueFilesErr } = await supabaseAdmin
      .from('issue_files')
      .select('file_url')
      .in('issue_id', issueIds)
    throwOnError(issueFilesErr, 'select issue files')
    for (const file of issueFiles || []) addStorageValue(buckets, file.file_url)
  }

  const { data: streamFiles, error: streamErr } = await supabaseAdmin
    .from('project_stream_posts')
    .select('file_url')
    .in('project_id', projectIds)
  throwOnError(streamErr, 'select stream attachments')
  for (const file of streamFiles || []) addStorageValue(buckets, file.file_url)

  const { data: projectFiles, error: projectFilesErr } = await supabaseAdmin
    .from('files')
    .select('file_url')
    .in('project_id', projectIds)
  throwOnError(projectFilesErr, 'select project files')
  for (const file of projectFiles || []) addStorageValue(buckets, file.file_url)

  const { data: channels, error: channelsErr } = await supabaseAdmin
    .from('message_channels')
    .select('id')
    .in('project_id', projectIds)
  throwOnError(channelsErr, 'select message channels for storage')
  ...
  if (channelIds.length) {
    ...
    throwOnError(messagesErr, 'select message attachments')
    for (const message of messages || []) addStorageValue(buckets, message.file_url)
  }
...
async function removeStoragePaths(
  ...
  for (const group of groups) {
    ...
    if (error) {
      throw new Error(`storage cleanup ${group.bucket}: ${error.message}`)
    }
  }
}
...
async function deleteProjectsByIds(
  ...
  const { data: orgSchedules, error: orgSchedulesErr } = await supabaseAdmin
    .from('progress_report_schedules')
    .select('id, included_project_ids')
    .not('included_project_ids', 'is', null)
  throwOnError(orgSchedulesErr, 'select organization report schedules')
  for (const schedule of orgSchedules || []) {
    const nextIds = (schedule.included_project_ids || []).filter(
      (id: string) => !projectIds.includes(id),
    )
    if (nextIds.length !== (schedule.included_project_ids || []).length) {
      throwOnError(
        (await supabaseAdmin
          .from('progress_report_schedules')
          .update({ included_project_ids: nextIds })
          .eq('id', schedule.id)).error,
        'remove project from organization report schedule',
      )
    }
  }

  throwOnError(
    (await supabaseAdmin.from('activity_log').delete().in('project_id', projectIds)).error,
    'activity_log',
  )
...
async function purgeSingleProject(
  ...
  const { data: latestLifecycle } = await supabaseAdmin
  ...
  const storageGroups = await collectProjectStoragePaths(supabaseAdmin, [projectId])
  ...
  await supabaseAdmin.from('project_lifecycle_events').insert({
    ...
    recipient_user_ids: latestLifecycle?.recipient_user_ids || [],
