...
async function deleteUserScopedData(
  ...
  throwOnError(
    (await supabaseAdmin.from('contacts').delete().eq('created_by_user_id', userId)).error,
    'contacts created_by',
  )
  throwOnError(
  ...
  // Clear user FKs that block auth deletion (no ON DELETE CASCADE).
  throwOnError(
...
serve(async (req) => {
  ...
  try {
    ...
    if (profile?.contact_id) {
      throwOnError(
        (await supabaseAdmin.from('contacts').delete().eq('id', profile.contact_id)).error,
        'profile contact',
      )
    }
